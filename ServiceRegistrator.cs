...
public class ServiceRegistrator : IPluginServiceRegistrator
{
    public void RegisterServices(IServiceCollection services, IServerApplicationHost host)
    {
        ...
        services.AddSingleton<NebulaBridgeCache>();
        services.AddSingleton<NebulaBridgeFileLog>();
        services.AddHostedService(sp => sp.GetRequiredService<NebulaBridgeFileLog>());
        // Registered before its users: hosted services stop in reverse order, so this one
        // stops last and work enqueued while other services are stopping is still owned.
        services.AddSingleton<BackgroundWork>();
        services.AddSingleton<DiscoveryActivity>();
        services.AddHostedService(sp => sp.GetRequiredService<BackgroundWork>());
        services.AddSingleton<AcquisitionJobStore>();
        services.AddSingleton<ProgressiveRangeCache>();
        services.AddSingleton<AcquisitionActivityTracker>();
        services.AddSingleton<CacheStorageSafety>();
        services.AddSingleton<AcquisitionCoordinator>();
        services.AddSingleton<AcquisitionImportService>();
        services.AddHostedService<AcquisitionImportRecoveryService>();
        services.AddHostedService<AcquisitionReconciliationService>();
        ...
        services.AddSingleton<HierarchyHydrationService>();
        services.AddHostedService<DiscoveryPromotionService>();
        services.AddSingleton<DiscoveryPrefetchService>();
        services.AddHostedService(sp => sp.GetRequiredService<DiscoveryPrefetchService>());
        services.AddSingleton<BridgeLibraryService>();
        services.AddSingleton<LibraryRepairService>();
        ...
        services.AddSingleton<IFlareSolverrSettings, PluginFlareSolverrSettings>();
        ...
        services.AddSingleton<IDebridProviderSettingsProvider, PluginDebridProviderSettingsProvider>();
        services.AddSingleton<DebridProviderHealthTracker>();
        services.AddSingleton<DebridProviderOrchestrator>();
        ...
        services.AddSingleton<RealDebridHttpClient>();
        services.AddSingleton<RealDebridStreamResolver>();
        services.AddSingleton<IDebridProvider>(sp => sp.GetRequiredService<RealDebridStreamResolver>());
        services.AddSingleton<NativeStreamProxyRegistry>();
        services.AddSingleton<NativeStreamProxyHttpClient>();
        services.AddSingleton<StreamAddonClient>();
        ...
        services
            .AddHttpClient(nameof(StreamAddonClient))
            .ConfigurePrimaryHttpMessageHandler(() =>
                new HttpClientHandler { AutomaticDecompression = System.Net.DecompressionMethods.All }
            )
            .ConfigureHttpClient(client =>
            {
                // The per-request timeout is the configured addon timeout; this only bounds a
                // client that was created outside the pipeline.
                client.Timeout = TimeSpan.FromSeconds(30);
                client.DefaultRequestHeaders.Accept.ParseAdd("application/json");
            });
        services
            .AddHttpClient(nameof(FlareSolverrClient))
            .ConfigureHttpClient(client => client.Timeout = TimeSpan.FromSeconds(75));
        services
        ...
        services
            .AddHttpClient(nameof(NebulaBridgeImageProvider))
