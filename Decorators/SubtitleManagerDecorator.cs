...
namespace NebulaBridge.Decorators
{
    public sealed class SubtitleManagerDecorator : ISubtitleManager
    {
        ...
        {
            var nebulabridgeFilename = video.IsNebulaBridge() ? video.NebulaBridgeData<string>("filename") : null;
            if (!string.IsNullOrEmpty(nebulabridgeFilename))
            {
                var originalPath = video.Path;
                video.Path = nebulabridgeFilename;
                try
                {
                    await _inner
        ...
        {
            if (video.IsNebulaBridge())
            {
                if (libraryOptions.SaveSubtitlesWithMedia)
                    _log.LogWarning(
                        "SaveSubtitlesWithMedia is enabled but subtitles cannot be saved alongside Nebula Bridge stream items. Disable it in the library settings to suppress this warning."
                    );

                libraryOptions.SaveSubtitlesWithMedia = false;

                // Jellyfin derives the subtitle save filename from video.Path.
                // For nebulabridge stream items the path is a URL, which produces garbage names.
                // Use the BehaviorHints.Filename stored in NebulaBridgeData if available.
                var nebulabridgeFilename = video.NebulaBridgeData<string>("filename");
                if (!string.IsNullOrEmpty(nebulabridgeFilename))
                {
                    var originalPath = video.Path;
                    video.Path = nebulabridgeFilename;
