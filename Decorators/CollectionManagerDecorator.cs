...
) : ICollectionManager
{
...
#if JELLYFIN_12
    // New in Jellyfin 12. Nebula does not own collection membership, so this is
    // a pass-through; the inner manager already applies the user's visibility.
    ...
    public async Task AddToCollectionAsync(Guid collectionId, IEnumerable<Guid> itemIds)
    {
        ...
        {
            var item = itemList[i];
            newChildren[originalLen + i] = item.CreateLinkedChild();

            log.LogDebug(
                "Adding item {Id} (Nebula Bridge={IsNebula Bridge}) to collection {Name}",
                item.Id,
                item.IsNebulaBridge(),
                collection.Name
            );
