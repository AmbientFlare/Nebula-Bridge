    ...
    : IDtoService
{
    ...
    // Jellyfin 12 added a trailing skipVisibilityCheck flag to this member. The
    // body is shared; only the signature and the forward differ per server line.
#if JELLYFIN_12
    ...
    ) => GetBaseItemDtosCore(items, options, user, owner, skipVisibilityCheck);
#else
    public IReadOnlyList<BaseItemDto> GetBaseItemDtos(
        IReadOnlyList<BaseItem> items,
        DtoOptions options,
        User? user = null,
        BaseItem? owner = null
    ) => GetBaseItemDtosCore(items, options, user, owner, false);
#endif

    private IReadOnlyList<BaseItemDto> GetBaseItemDtosCore(
        IReadOnlyList<BaseItem> items,
        DtoOptions options,
        User? user,
        BaseItem? owner,
