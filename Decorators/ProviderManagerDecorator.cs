...
) : IProviderManager
{
    ...
    )
    {
        ...
        {
            log.LogDebug(
                "SaveImage intercepted: item={Id} name={Name} type={ImageType} index={Index}",
                item.Id,
                item.Name,
                type,
                imageIndex
            ...
            // Always persist the URL at the nebulabridge fake path so it can be resolved on demand
            ...
            if (NebulaBridgePlugin.Instance?.Configuration.LazyImages == true)
            {
...
#if JELLYFIN_12
    // Jellyfin 12 added an includeDisabled overload. Nebula does not filter the
    // provider list, so both overloads forward straight through.
