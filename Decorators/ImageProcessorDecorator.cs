...
{
    private string NebulaBridgeImagesDir => Path.Combine(appPaths.DataPath, "nebulabridge", "images");
    ...
    // Works for any path that has a .url sidecar (nebulabridge items, persons, studios, etc.).
    // Falls back to the nebulabridge fake path for items whose DB path reverted to the Jellyfin
                ...
                if (urlFile is not null)
                {
                    ...
                    try
                    {
                        ...
                        await libraryManager.Value
                            ...
                            .ConfigureAwait(false);
                        log.LogDebug(
                            "ImageProcessor: resolved image for {Id} type={Type}",
                            options.Item.Id,
                            options.Image.Type
                        );
                    }
                    ...
                    {
                        log.LogWarning(
                            "ImageProcessor: download failed ({ErrorType})",
                            ex.GetType().Name
                        );
                    }
                }
    ...
    // falling back to the nebulabridge fake path for this item + image type.
    private string? ResolveUrlFile(string imagePath, ImageProcessingOptions options)
    {
        ...
        var fallback =
            Path.Combine(NebulaBridgeImagesDir, options.Item.Id.ToString("N"), fileName) + ".url";
