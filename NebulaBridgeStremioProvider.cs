...
public class NebulaBridgeStremioProvider(
    string baseUrl,
    IHttpClientFactory http,
    ILogger<NebulaBridgeStremioProvider> log
)
{
    ...
    {
        var c = http.CreateClient(nameof(NebulaBridgeStremioProvider));
    ...
    private async Task<T?> GetJsonAsync<T>(string url)
    {
        ...
        log.LogDebug("Requesting Stremio-compatible endpoint at {Origin}", origin);
        ...
        try
        {
            ...
            {
                log.LogDebug(
                    "Stremio-compatible request to {Origin} failed with {StatusCode} {ReasonPhrase}",
                    origin,
                    resp.StatusCode,
                    resp.ReasonPhrase
                );
        ...
        {
            log.LogDebug(
                "Stremio-compatible request to {Origin} failed with HTTP status {StatusCode}",
                origin,
                ex.StatusCode
            );
            throw;
        ...
        {
            log.LogDebug(
                "Stremio-compatible request to {Origin} failed ({ErrorType})",
                origin,
                ex.GetType().Name
            );
            throw new InvalidOperationException(
                $"Stremio-compatible request failed ({ex.GetType().Name}).",
                ex
            );
        }
    }

    private static string GetSafeOrigin(string url) =>
    ...
    )
    {
        ...
        try
        {
            using var client = http.CreateClient(nameof(NebulaBridgeStremioProvider));
    ...
    )
    {
        ...
        return r?.Metas
            ?? throw new HttpRequestException("Catalog response was missing or malformed.");
    }
...
[JsonConverter(typeof(StremioResourceJsonConverter))]
...
public sealed class StremioResourceJsonConverter : JsonConverter<StremioResource>
{
    ...
    )
    {
        ...
        {
            return new StremioResource { Name = reader.GetString() ?? string.Empty };
        }

        if (reader.TokenType != JsonTokenType.StartObject)
        {
            throw new JsonException("A Stremio resource must be a string or object.");
        }

        using var document = JsonDocument.ParseValue(ref reader);
        var root = document.RootElement;
        return new StremioResource
        {
            Name = GetString(root, "name"),
            Types = GetStrings(root, "types"),
            IdPrefixes = GetStrings(root, "idPrefixes"),
        };
    }
    ...
    private static List<string> GetStrings(JsonElement root, string propertyName)
    {
        if (
            !TryGetProperty(root, propertyName, out var property)
...
public class StremioStream
{
    ...
    public Guid GetGuid(string mediaId)
    {
        ...
        var bytes = System.Text.Encoding.UTF8.GetBytes($"{mediaId}\n{key}");
