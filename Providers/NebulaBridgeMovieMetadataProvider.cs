...
public sealed class NebulaBridgeMovieMetadataProvider(
    ILogger<NebulaBridgeMovieMetadataProvider> log,
...
) : IRemoteMetadataProvider<Movie, MovieInfo>, IHasOrder
{
    ...
    )
    {
        ...
        if (id is null)
        {
            log.LogDebug("Nebula Bridge movie metadata provider: no usable ID for {Name}", info.Name);
            return result;
        }
        ...
        {
            meta = await metadata
                .GetMetaAsync(configuration, id, StremioMediaType.Movie, cancellationToken)
                .ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            log.LogWarning(ex, "Nebula Bridge movie metadata provider: failed to fetch meta for {Id}", id);
            return result;
        }
    ...
    )
    {
        ...
        catch (Exception ex)
        {
            log.LogWarning(
                ex,
                "Nebula Bridge movie metadata provider: search failed for {Name}",
                searchInfo.Name
            );
            return [];
        }
