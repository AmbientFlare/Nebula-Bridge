...
public sealed class NebulaBridgeSeasonMetadataProvider(
    ILogger<NebulaBridgeSeasonMetadataProvider> log,
        ...
        IHasOrder
{
    ...
    )
    {
        ...
        if (string.IsNullOrWhiteSpace(seriesImdbId))
        {
            log.LogDebug("Nebula Bridge season metadata provider: no series IMDB id for {Name}", info.Name);
            return result;
        }
        ...
        catch (Exception ex)
        {
            log.LogWarning(
                ex,
                "Nebula Bridge season metadata provider: failed to fetch series meta for {Id}",
                seriesImdbId
            );
            return result;
        }
