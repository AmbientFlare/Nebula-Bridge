...
namespace NebulaBridge.Providers
{
    public sealed class SubtitleProvider : ISubtitleProvider
    {
        ...
        public string Name => "Nebula Bridge Subtitles";
        ...
        )
        {
            ...
            try
            {
                // Prefer the filename stored in NebulaBridgeData (set from BehaviorHints.Filename during stream
                ...
                var streamItem = _library
                    ...
                    .FirstOrDefault();
                var nebulabridgeFilename = streamItem?.NebulaBridgeData<string>("filename");

                if (!string.IsNullOrEmpty(nebulabridgeFilename))
                {
                    filename = nebulabridgeFilename;
                    _log.LogDebug("Using Nebula Bridge data filename: {Filename}", filename);
                }
                else if (
        ...
        private static string ListCacheKey(string id, StremioMediaType mediaType) =>
            $"nebulabridge:subtitles:{mediaType.ToString().ToLower()}/{id}";

        private static string SubCacheKey(string id) => $"nebulabridge:subtitle:{id}";
    }
}
