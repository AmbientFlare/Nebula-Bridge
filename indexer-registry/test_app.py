...
class CatalogAppTests(unittest.TestCase):
    ...
    def test_flaresolverr_definition_is_compatible_and_publishable(self) -> None:
        document = yaml.safe_load(fixture("showrss.yml"))
        document["settings"] = [
            {
                "name": "flaresolverr",
        ...
        requires_solver = yaml.safe_dump(document, sort_keys=False).encode()
        self.sync({"showrss.yml": requires_solver})

        item = self.store.list_admin()["indexers"][0]
        self.assertEqual("requires_flaresolverr", item["compatibilityStatus"])
        self.assertTrue(item["compatible"])
        self.assertTrue(item["requiresFlareSolverr"])
        self.assertTrue(item["flareSolverrOnly"])
        self.store.set_published("showrss-yml", True)
        manifest_item = self.store.manifest()["indexers"][0]
        self.assertTrue(manifest_item["requires_flaresolverr"])

    def test_flaresolverr_does_not_mask_other_incompatibility(self) -> None:
        document = yaml.safe_load(fixture("showrss.yml"))
        document["settings"] = [{"name": "flaresolverr", "type": "info_flaresolverr"}]
        document["search"]["rows"]["filters"].append({"name": "strdump"})
        self.sync({"mixed.yml": yaml.safe_dump(document, sort_keys=False).encode()})

        item = self.store.list_admin()["indexers"][0]
        self.assertEqual("unsupported", item["compatibilityStatus"])
        self.assertFalse(item["compatible"])
        self.assertFalse(item["flareSolverrOnly"])
