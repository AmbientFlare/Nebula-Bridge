...
def classify(document: dict[str, Any]) -> dict[str, Any]:
    ...
    flare_solverr_only = bool(
        requires_flaresolverr and not authentication and not configuration and not unsupported
    )
    if authentication:
        ...
        status = "unsupported"
    elif flare_solverr_only:
        status = "requires_flaresolverr"
    else:
        status = "compatible"
    notes = authentication + configuration + unsupported
    if flare_solverr_only:
        notes.append("FlareSolverr endpoint required")
    return {
        "compatibilityStatus": status,
        "compatible": status in ("compatible", "requires_flaresolverr"),
        "requiresAuthentication": bool(authentication),
        "requiresConfiguration": bool(configuration),
        "requiresFlareSolverr": requires_flaresolverr,
        "flareSolverrOnly": flare_solverr_only,
