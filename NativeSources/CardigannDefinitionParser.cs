...
public sealed class CardigannDefinitionParser
{
    ...
    )
    {
        ...
        var definition = new IndexerDefinition
        {
            ...
            RequiresFlareSolverr = RequiresFlareSolverr(document),
        };
    ...
    private static bool RequiresFlareSolverr(JsonObject document) =>
        (document["settings"]?.AsArray() ?? [])
            .OfType<JsonObject>()
            .Any(setting => string.Equals(
                Text(setting, "type"),
                "info_flaresolverr",
