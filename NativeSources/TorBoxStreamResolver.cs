...
) : IDebridProvider
{
    ...
    )
    {
        ...
        try
        {
            ...
            var files = torrent.Files.Select(ToDebridFile).ToList();
            ...
            NebulaBridgeFileLog.Write(
                ...
                fields: new Dictionary<string, object?>
                {
                    ["provider"] = Id,
                    ["source"] = candidate.SourceId,
                    ["fileSize"] = file.SizeBytes ?? candidate.SizeBytes,
                });
            return new DebridPlaybackResult(
                new NativeResolvedStream(
                    ...
                    new DebridCompletedFileHandle(
                        Id,
                        torrentId.Value.ToString(),
                        file.Id.ToString(),
                        Path.GetFileName(file.Name),
                        file.SizeBytes ?? candidate.SizeBytes,
                        hash,
    ...
    )
    {
        if (!string.Equals(handle.ProviderId, Id, StringComparison.OrdinalIgnoreCase)
            || !long.TryParse(handle.RemoteItemId, out var torrentId)
            || !long.TryParse(handle.FileId, out var fileId)) return null;
        var settings = Settings;
        if (!settings.Configured) return null;
        var url = await RequestDownloadLinkAsync(torrentId, fileId, settings.Credential, cancellationToken).ConfigureAwait(false);
        if (url is null || url.Scheme != Uri.UriSchemeHttps || !string.IsNullOrEmpty(url.UserInfo)) return null;
        await targetValidator.ValidateAsync(url, cancellationToken).ConfigureAwait(false);
        return new DebridCompletedFileSource(url, handle.ExpectedLength, SupportsRanges: true);
    }
    ...
    public async Task<NativeSourceFailure?> TestConnectionAsync(CancellationToken cancellationToken)
    {
        var settings = Settings;
        if (!settings.Configured)
            return new NativeSourceFailure(Id, "TorBox is not configured.", Name, "configuration", "not_configured");
        try
        {
            using var request = CreateAuthorizedRequest(HttpMethod.Get, "user/me?settings=false", settings.Credential);
            var data = await SendJsonElementAsync(request, cancellationToken).ConfigureAwait(false);
            if (data is null || data.Value.ValueKind != JsonValueKind.Object)
                return new NativeSourceFailure(Id, "TorBox returned no account data.", Name, "account", "api_error");
            if (data.Value.TryGetProperty("plan", out var plan) && plan.ValueKind == JsonValueKind.Number && plan.GetInt32() == 0)
                return new NativeSourceFailure(Id, "The TorBox account has no active plan.", Name, "account", "premium_required");
            return null;
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
        ...
        catch (Exception ex) when (IsExpectedFailure(ex))
        {
            ...
            return new NativeSourceFailure(Id, "TorBox account check failed.", Name, "account", FailureReason(ex));
        }
    }
    ...
    private static string FailureReason(Exception exception) =>
        exception switch
        {
            ...
            TorBoxApiException apiException when apiException.StatusCode is 429 => "rate_limited",
            TorBoxApiException apiException when apiException.StatusCode >= 500 => "provider_unavailable",
