...
/// <summary>
/// Palco API - Simple key-value cache migrated to NebulaBridge.
...
public class PalcoCacheController(ILogger<PalcoCacheController> logger) : ControllerBase
{
    ...
    // Access the service via NebulaBridgePlugin instance or injection
    private PalcoCacheService? Cache => NebulaBridgePlugin.Instance?.PalcoCache;
    ...
    )
    {
        ...
        logger.LogInformation("[Nebula Bridge] Palco Registration request received: {Id}", request.Id);
        ...
        if (!index.Contains(requestId))
        {
            ...
            logger.LogInformation("[Nebula Bridge] Palco Request added to index: {Id}", requestId);
        }
                    ...
                    if (!string.IsNullOrEmpty(adminEmail))
                    {
                        ...
                        var mail = new MailMessage
                        {
                            From = new MailAddress(
                                smtp.FromAddress ?? smtp.Username ?? adminEmail,
                        ...
                        logger.LogInformation(
                            "[Nebula Bridge] Palco Admin notification sent to {AdminEmail} for registration: {Id}",
                            adminEmail,
                            requestId
                        );
                    }
        ...
        {
            logger.LogWarning(ex, "[Nebula Bridge] Palco Failed to send admin notification email");
        }
    ...
    )
    {
        ...
        logger.LogInformation("[Nebula Bridge] Palco Saved: {Key} in {Ns}", key, ns);
    ...
    public ActionResult Delete([FromRoute] string key, [FromQuery] string ns = "")
    {
        ...
        if (key.StartsWith("request-") && ns == RegistrationNs)
        {
            ...
            if (!string.IsNullOrEmpty(indexJson))
            {
                ...
                if (index.Remove(requestId))
                {
                    Cache.Set("requests-index", JsonSerializer.Serialize(index), 0, RegistrationNs);
                    logger.LogInformation(
                        "[Nebula Bridge] Palco Request removed from index: {Id}",
                        requestId
                    );
                }
            }
        ...
        logger.LogInformation(
            "[Nebula Bridge] Palco Deleted: {Key} from {Ns}, success={Deleted}",
            key,
            ns,
            deleted
        );
    ...
    public async Task<ActionResult> SendEmail([FromBody] EmailRequest request)
    {
        try
        {
            ...
            await client.SendMailAsync(mail);
            logger.LogInformation("[Nebula Bridge] Palco Email sent to {To}", request.To);
            return Ok(new { success = true });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "[Nebula Bridge] Palco Email failed to {To}", request.To);
