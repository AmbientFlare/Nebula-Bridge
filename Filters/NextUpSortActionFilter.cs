        ...
        IOrderedFilter
{
    ...
    )
    {
        ...
        var page = SortAndPage(result.Items, requestedStart, requestedLimit);
        result.TotalRecordCount = page.TotalCount;
    ...
    internal static (BaseItemDto[] Items, int TotalCount) SortAndPage(
        IEnumerable<BaseItemDto> items,
        int startIndex,
        int? limit,
        DateTime? utcNow = null
    ...
    {
        var sorted = SortNewestFirst(items, utcNow).ToList();
        IEnumerable<BaseItemDto> page = sorted.Skip(Math.Max(0, startIndex));
        if (limit is > 0)
        {
            page = page.Take(limit.Value);
        }

        return (page.ToArray(), sorted.Count);
    }
