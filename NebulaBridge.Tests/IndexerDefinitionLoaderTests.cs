...
public sealed class IndexerDefinitionLoaderTests
{
    ...
    [Fact]
    public async Task FlareSolverrDefinitionsAreVisibleButUnavailableUntilConfigured()
    {
        var yaml = File.ReadAllText(CardigannTestSupport.FixturePath("showrss.yml"))
            .Replace(
                "settings: []",
        ...
        var preferences = new CardigannTestSupport.MemoryPreferenceStore(["showrss-yml"]);
        var loader = CardigannTestSupport.CreateLoader(provider, preferences);
        await loader.RefreshAsync(CancellationToken.None);

        var unavailable = Assert.Single(loader.GetAllSummaries());
        Assert.True(unavailable.Compatible, unavailable.Error);
        Assert.True(unavailable.RequiresFlareSolverr);
        Assert.False(unavailable.Available);
        Assert.False(unavailable.Enabled);
        Assert.Equal("requires_flaresolverr", unavailable.State);
        Assert.Contains("Configure a FlareSolverr endpoint", unavailable.Error);
        Assert.Empty(loader.GetEnabledDefinitions());
        Assert.False(loader.SetEnabled(unavailable.Id, true));
        Assert.Equal(0, loader.GetStatus().AvailableCount);

        loader = CardigannTestSupport.CreateLoader(
            provider,
            preferences,
        ...
        var available = Assert.Single(loader.GetAllSummaries());
        Assert.True(available.Available);
        Assert.True(available.Enabled);
        Assert.Equal("enabled", available.State);
        Assert.True(loader.GetRequired(available.Id).RequiresFlareSolverr);
        Assert.Equal(1, loader.GetStatus().AvailableCount);
    }
