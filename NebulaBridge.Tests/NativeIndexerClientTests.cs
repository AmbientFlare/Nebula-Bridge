...
public sealed class NativeIndexerClientTests
{
    ...
    [Fact]
    public async Task AnAbsoluteApiPathIsRequestedOnceNotOncePerMirror()
    {
        var definition = ApiDefinition("absolute-api-once");
        var requests = 0;
        var client = CardigannTestSupport.CreateClient(_ =>
        {
            requests++;
        ...
        await Assert.ThrowsAsync<HttpRequestException>(() =>
            client.SearchAsync(definition, new NativeMediaQuery("Film"), CancellationToken.None));

        Assert.Equal(1, requests);
    }

    [Fact]
    public async Task ARateLimitStopsTheMirrorWalkAndCoolsTheIndexerDown()
    {
        var definition = MirrorDefinition("rate-limited-mirrors");
        var requests = 0;
        var client = CardigannTestSupport.CreateClient(_ =>
        {
            requests++;
        ...
        // Within the cooldown the indexer is skipped without touching the network.
    ...
    [Fact]
    public async Task AMirrorThatIsDownStillFallsThroughToTheNext()
    {
        var definition = MirrorDefinition("mirror-fallthrough");
        var hosts = new List<string>();
        var client = CardigannTestSupport.CreateClient(request =>
        {
            hosts.Add(request.RequestUri!.Host);
            return request.RequestUri.Host == "primary.example"
                ? new HttpResponseMessage(HttpStatusCode.BadGateway)
                : Ok("""[{"id":"1","name":"Film","info_hash":"0123456789abcdef0123456789abcdef01234567"}]""");
        });

        Assert.Single(await client.SearchAsync(definition, new NativeMediaQuery("Film"), CancellationToken.None));
        Assert.Equal(["primary.example", "legacy.example"], hosts);
    }

    private static IndexerDefinition ApiDefinition(string id) =>
        new()
        {
            Id = id,
            Name = id,
            Type = "public",
            Links = ["https://primary.example/", "https://legacy.example/", "https://third.example/"],
            SourcePath = id + ".yml",
            Document = Object("""
                {
                  "settings":[{"name":"apiurl","type":"text","default":"api.example"}],
                  "search":{
                    "paths":[{"path":"https://{{ .Config.apiurl }}/q?q={{ .Keywords }}","response":{"type":"json"}}],
                    "rows":{"selector":"$"},
                    "fields":{"title":{"selector":"name"},"infohash":{"selector":"info_hash"}}
                  }
                }
                """),
        };

    private static IndexerDefinition MirrorDefinition(string id) =>
        new()
        {
            Id = id,
            Name = id,
            ...
            Document = Object("""
                {
                  "search":{
                    "paths":[{"path":"q?q={{ .Keywords }}","response":{"type":"json"}}],
                    "rows":{"selector":"$"},
                    "fields":{"title":{"selector":"name"},"infohash":{"selector":"info_hash"}}
                  }
                }
