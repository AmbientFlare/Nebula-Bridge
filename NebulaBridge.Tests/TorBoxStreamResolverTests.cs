...
public sealed class TorBoxStreamResolverTests
{
    ...
    [Fact]
    public async Task CompletedFileHandleRegeneratesAnEphemeralSourceWithoutPersistingAUrl()
    {
        var handler = new RecordingHandler(
            [JsonResponse("""{"success":true,"data":"https://cdn.torbox.test/fresh.mkv?signature=temporary"}""")]
        );
        using var http = new TorBoxHttpClient(handler);
        var provider = CreateProvider(http, new(true, ApiToken));
        var handle = new DebridCompletedFileHandle("torbox", "42", "7", "movie.mkv", 900);

        var source = await provider.ResolveCompletedFileSourceAsync(handle, CancellationToken.None);

        Assert.NotNull(source);
        Assert.Equal(900, source.ExpectedLength);
        Assert.True(source.SupportsRanges);
        Assert.Contains("torrent_id=42", handler.Requests[0].Uri.Query, StringComparison.Ordinal);
        Assert.Contains("file_id=7", handler.Requests[0].Uri.Query, StringComparison.Ordinal);
        Assert.DoesNotContain("signature", handle.ToString(), StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static TorBoxStreamResolver CreateProvider(
        ...
        new(
            http,
            new StaticSettingsProvider(new DebridProviderSettings("torbox", settings.Enabled, 1, settings.ApiToken)),
    ...
    private sealed record TorBoxResolverSettings(bool Enabled, string ApiToken);

    private sealed class StaticSettingsProvider(DebridProviderSettings settings)
        : IDebridProviderSettingsProvider
    {
        public DebridProviderSettings GetSettings(string providerId) =>
            string.Equals(providerId, settings.ProviderId, StringComparison.OrdinalIgnoreCase)
                ? settings
                : new DebridProviderSettings(providerId, false, 100, string.Empty);
    }
