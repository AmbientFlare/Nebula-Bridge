...
)
{
    ...
    {
        var cacheKey = TmdbCacheKey(mediaType, id);
        if (cache.Get<StremioMeta>(cacheKey) is { } cached)
        {
            return cached;
        }
        ...
        if (meta is not null)
        {
            ...
            cache.Set(TmdbCacheKey(mediaType, $"tmdb:{tmdbId}"), meta, CacheTtl);
        }

        return meta;
    }

    private static string TmdbCacheKey(StremioMediaType mediaType, string id) =>
        $"tmdb:meta:{mediaType}:{id.Trim().ToLowerInvariant()}";
    ...
    )
    {
        ...
        using var document = await GetDocumentAsync(
                $"{path}?query={Uri.EscapeDataString(query)}&include_adult=true",
                cancellationToken
            )
            .ConfigureAwait(false);
