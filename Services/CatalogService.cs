...
)
{
        ...
        foreach (var definition in traktClient.GetCatalogDefinitions())
        {
            ...
            if (existing is null)
            {
                existing = new CatalogConfig
                {
                    ...
                    Enabled = IsDefaultEnabledSource(definition),
                    MaxItems = 0,
                    CreateCollection = false,
                    Url = definition.Path,
                };
    ...
    public void UpdateCatalogConfig(CatalogConfig updatedConfig)
    {
        ...
        if (existing != null)
        {
            ...
            existing.SeparateLibrary = updatedConfig.SeparateLibrary;
    ...
    /// <summary>
    /// Only the public Trakt Trending/Popular feeds start switched on: they are what fill the
    /// shared Movies and Shows libraries out of the box. Everything personal stays opt-in.
    /// </summary>
    internal static bool IsDefaultEnabledSource(TraktCatalogDefinition definition) =>
        !definition.RequiresAccount
        && (
            definition.Id.Contains("trending", StringComparison.OrdinalIgnoreCase)
            || definition.Id.Contains("popular", StringComparison.OrdinalIgnoreCase)
        );
