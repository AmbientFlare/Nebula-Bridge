...
namespace NebulaBridge.Services
{
    ...
    /// </summary>
    public class NebulaBridgeJavaScriptRegistrationService(
        IOptionsMonitor<PluginConfiguration> optionsMonitor,
        ILogger<NebulaBridgeJavaScriptRegistrationService> logger
    ) : IHostedService, IDisposable
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            // Also subscribe to plugin-level configuration update events (fired by NebulaBridgePlugin.UpdateConfiguration)
            try
            {
                NebulaBridgePlugin.ConfigurationChanged += OnConfigChanged;
            }
            catch (Exception)
            ...
            try
            {
                var cfg = NebulaBridgePlugin.Instance?.Configuration ?? optionsMonitor.CurrentValue;
        ...
        private void RegisterBundledScripts()
        {
            ...
            try
            {
                ...
                var pluginId = NebulaBridgePlugin.Instance?.Id.ToString() ?? "nebulabridge-plugin";
                var pluginName = NebulaBridgePlugin.Instance?.Name ?? "Nebula Bridge";
                var pluginVersion =
                    NebulaBridgePlugin.Instance?.GetType().Assembly.GetName().Version?.ToString()
                    ?? "unknown";
