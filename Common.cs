...
public sealed class KeyLock
{
    ...
    )
    {
        ...
        {
            // Keep the semaphore registered. Removing it after Release creates a race where a
            // waiter can attach to the old semaphore while a later caller receives a new one.
            sem.Release();
        }
    }
...
public static class ActionContextExtensions
{
    ...
    {
        foreach (var (key, raw) in ctx.ActionArguments)
        {
            ...
            if (raw is string value && Guid.TryParse(value, out guid) && guid != Guid.Empty)
            {
                userId = guid;
                return true;
            }
    ...
    public static bool TryGetUserId(this HttpContext ctx, out Guid userId)
    {
        ...
        var claimUserId = ctx.User.Claims
            ...
            ?.Value;
        var userIdStr = !string.IsNullOrWhiteSpace(claimUserId)
            ? claimUserId
...
public static class BaseItemExtensions
{
    ...
    public static bool HasStreamTag(this BaseItem item)
    {
        return item.Tags is not null
            && (
                ...
                || item.Tags.Contains(
                    NebulaBridgeManager.LegacyStreamTag,
                    StringComparer.OrdinalIgnoreCase
                )
            );
    }
    ...
    public static bool HasTreeSyncedTag(this BaseItem item) =>
        ...
        && (
            ...
            || item.Tags.Contains(
                NebulaBridgeManager.LegacyTreeSyncedTag,
                StringComparer.OrdinalIgnoreCase
            )
        );
