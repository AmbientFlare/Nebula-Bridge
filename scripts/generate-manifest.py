...
def main() -> None:
    ...
    # Trim per ABI stream, not globally: Nebula Bridge advertises the Jellyfin
    # 10.11 line and the Jellyfin 12 line from this one manifest, and a global
    # cap would let a busy stream evict the other one's only entry -- which
    # silently strands every server on that server line.
    kept: list = []
    seen: dict = {}
    for entry in versions:
        stream = entry.get("targetAbi", "")
        seen[stream] = seen.get(stream, 0) + 1
        if seen[stream] <= 25:
            kept.append(entry)
    plugin["versions"] = kept
